...
export default function ProductDetails() {
  ...
  let originalPrice = price * 1.15; // default fallback
      ...
      <div className="bg-[#fdfaf2] border-y border-[#e8dcc4] py-8 mt-20 sm:mt-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            <button
              onClick={() => router.back()}
              className="flex items-center justify-center h-10 w-10 rounded-full border border-[#e8dcc4] bg-white text-brand-charcoal hover:bg-[#FAF6EE] hover:text-brand-orange hover:border-brand-orange transition-all cursor-pointer shadow-3xs"
              title="Go Back"
            >
            ...
            <div className="md:col-span-7 flex flex-col gap-5">
              ...
              <div className="flex items-baseline gap-3 pb-2">
                ...
                {originalPrice > price && (
                  <>
                    ...
                    <span className="text-[10px] font-bold text-red-500 bg-red-50 border border-red-100 rounded-md px-2 py-0.5 ml-1 animate-pulse">
                      {discountPercent}% OFF
                    </span>
                  </>
                )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {sortWeights(Object.keys(product.prices)).map((w) => {
              ...
              <div className="border-t border-brand-beige pt-4 flex flex-wrap justify-between items-center gap-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex gap-2">
                    <button onClick={shareOnTwitter} className="w-8 h-8 bg-black text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity cursor-pointer shadow-3xs" title="Share on X (Twitter)">
                      <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
                        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                      </svg>
                    </button>
                    <button onClick={shareOnFacebook} className="w-8 h-8 bg-[#1877F2] text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity cursor-pointer shadow-3xs" title="Share on Facebook">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                      </svg>
                    </button>
                    <button onClick={shareOnPinterest} className="w-8 h-8 bg-[#E60023] text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity cursor-pointer shadow-3xs" title="Share on Pinterest">
                      <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                        <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.966 1.406-5.966s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.211-.174.263-.403.156-1.503-.707-2.441-2.92-2.441-4.706 0-3.839 2.79-7.365 8.047-7.365 4.225 0 7.507 3.013 7.507 7.036 0 4.199-2.645 7.579-6.321 7.579-1.233 0-2.397-.64-2.793-1.4l-.763 2.909c-.275 1.054-.97 2.378-1.442 3.149 1.125.346 2.316.536 3.551.536 6.621 0 11.986-5.367 11.986-11.987C24.007 5.368 18.64 0 12.017 0z"/>
                      </svg>
                    </button>
                    <button onClick={shareOnWhatsApp} className="w-8 h-8 bg-[#25D366] text-white rounded-full flex items-center justify-center hover:opacity-80 transition-opacity cursor-pointer shadow-3xs" title="Share on WhatsApp">
                      <svg className="w-4.5 h-4.5 fill-current" viewBox="0 0 24 24">
                        <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.513 2.266 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.455L0 24zm6.59-4.846c1.6.95 3.197 1.45 4.817 1.452 5.43 0 9.85-4.403 9.853-9.83.002-2.628-1.017-5.097-2.872-6.957-1.854-1.859-4.322-2.882-6.953-2.883-5.438 0-9.86 4.404-9.863 9.83-.001 1.76.471 3.477 1.367 4.982L2.005 21.9l4.642-1.746z"/>
                      </svg>
                    </button>
                  </div>
                </div>
