...
export default function AdminCategories({ categories, setCategories }: { categories: any[], setCategories: any }) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append('bucket', 'products'); // Reusing products bucket as per existing logic
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-4">
                  {imageUrl && <img src={img.thumbnail(imageUrl)} alt="Category" className="max-w-[64px] max-h-[64px] w-auto h-auto rounded-lg border border-brand-beige" />}
            ...
            {categories.map((cat) => (
              <tr key={cat.id || cat.slug} className="border-b border-brand-beige/50 hover:bg-brand-cream/10 transition-colors">
                <td className="py-3 px-4">
                  {cat.image_url ? (
                    <img src={img.thumbnail(cat.image_url)} alt={cat.name} className="max-w-[40px] max-h-[40px] w-auto h-auto rounded-md border border-brand-beige shadow-xs" />
                  ) : (
